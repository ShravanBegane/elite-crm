...
if($num_rows == 0){
		...
		$sql = "INSERT INTO `vtiger_leaddetails` (`leadid`, `lead_no`, `email`, `firstname`, `lastname`,`emailoptout`,`leadsource`) VALUES
		('".$crmid."', '".$prev_inv_no."', '".$email."', '".$firstname."', '".$lastname."', '0', '".$leadsource."')";//".$leadsource."
		$result = $adb->pquery($sql);
		...
		$sql = "INSERT INTO `vtiger_leadaddress` (`leadaddressid`,`code`,`phone`,`city`,`state`) VALUES
		('".$crmid."','".$group_role."', '".$phone."', '".$city."', '".$state."')";
		$result = $adb->pquery($sql);
...
$sql = "INSERT INTO `vtiger_crmentity` (`crmid`, `smcreatorid`, `smownerid`, `modifiedby`, `setype`, `description`, `createdtime`, `modifiedtime`, `viewedtime`, `status`, `version`, `presence`, `deleted`, `smgroupid`, `source`, `label`) VALUES
('".$crmidD."', 1,'".$ownerid ."', 1, 'Documents', '".$commnts."', '".date("Y-m-d h:i:s")."', '".date("Y-m-d h:i:s")."', NULL, NULL, 0, 1, 0, 0, 'CRM', '')";
$result = $adb->pquery($sql);
