		...
		<div class="container-fluid global-nav" >
			<div class="row">
				...
				<div class="navbar-header paddingTop5">
					<button type="button" class="navbar-toggle collapsed border0" data-toggle="collapse" onclick="setTimeout(marginetop, 100)" id="toggle1" data-target="#navbar" aria-expanded="false">
						<i class="fa fa-th"></i>
					</button>
					<button type="button" class="navbar-toggle collapsed border0" data-toggle="collapse" onclick="setTimeout(marginetop, 100)" id="toggle2" data-target="#search-links-container" aria-expanded="false">
				...
				<div class="col-sm-3">
					<div id="search-links-container" class="search-links-container collapse navbar-collapse">
						<div class="search-link">
							<span class="fa fa-search" aria-hidden="true" style='color:black;'></span>
							<input class="keyword-input" type="text" placeholder="{vtranslate('LBL_TYPE_SEARCH')}" value="{$GLOBAL_SEARCH_VALUE}">
							<span id="adv-search" class="adv-search fa fa-chevron-circle-down pull-right cursorPointer" aria-hidden="true" style='color:black;'></span>
						</div>
					</div>
				</div>
												...
												{if $moduleModel->isPermitted('CreateView') || $moduleModel->isPermitted('EditView')}
													...
													{if $quickCreateModule == '1'}
														{if $count % 3 == 0}
															...
															{else if $singularLabel == 'SINGLE_Locationmap' || $singularLabel == 'SINGLE_EmployeeRoaster'}
																<div class="{if $hideDiv}create_restricted_{$moduleModel->getName()} hide{else}col-lg-4 col-xs-4{/if}">
																	<a id="menubar_quickCreate_{$moduleModel->getName()}" class="quickCreateModule" data-name="{$moduleModel->getName()}"
																	   data-url="" href="{$moduleModel->getDefaultUrl()}&app=RIGHTCLEAN">
																		{$moduleModel->getModuleIcon()}
